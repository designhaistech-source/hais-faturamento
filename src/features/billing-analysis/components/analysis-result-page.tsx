        ...
        <div className="space-y-5">
          <section aria-labelledby="summary-title" className="space-y-2">
            ...
            <p className="flex flex-wrap items-center gap-x-2 gap-y-1 text-sm text-foreground">
              ...
              <SummaryPart icon={<Check className="h-3.5 w-3.5 text-success" aria-hidden="true" />}>
                {summary.matches} {summary.matches === 1 ? "conformidade" : "conformidades"}
              </SummaryPart>
              <SummaryPart icon={<X className="h-3.5 w-3.5 text-destructive" aria-hidden="true" />}>
                {summary.divergences} {summary.divergences === 1 ? "divergência" : "divergências"}
              </SummaryPart>
          ...
          {conforming.length > 0 && (
            <section className="rounded-xl border border-border">
              ...
              {showConforming && (
                <ul id="conforming-items" className="divide-y divide-border border-t border-border">
                  {conforming.map((item) => (
                    <li key={item.id} className="space-y-2 px-4 py-3">
                      <div className="flex min-w-0 items-start justify-between gap-3">
                        <div className="min-w-0">
                          <p className="font-mono text-xs text-muted-foreground">{item.code}</p>
                          <p className="break-words text-sm text-foreground">{item.description}</p>
                        </div>
                        <ItemStatusBadge status={item.status} />
                      </div>
